...
type SqlClient[D any] struct {
	*graviflow.AppInjector[D]
	...
	DriverName       graviflow.Config `config:"driver.name,str" default:"postgres" usage:"Driver name to use in the SQL client"`
	ConnectionString graviflow.Config `config:"connection.string,str" usage:"Connection string to connect to SQL database"`
}
